...
import ImageViewBlob from '@/components/atoms/CoreUploadBox/ImageViewBlob'
...
export const uploadImageFile = async (dataBody: any): Promise<any> => {
  const { data } = await authUAAAPI({
    ...
    url: '/cms/api/v1/sub-menu-configs/images',
    data: dataBody,
    isFileUpload: true,
  })
            ...
            if (file) {
              if (listFileAllow.some((v) => file.type.includes(v))) {
                ...
                if (file.size <= 20000000) {
                  ...
                  console.log('runThat', formData, file)
                  ...
                  errorMsg('File must not exceeds 20MB')
                  setLoading(false)
                  setIsUploading(false)
                }
              } else {
